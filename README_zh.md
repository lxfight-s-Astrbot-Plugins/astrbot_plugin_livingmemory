...
**版本**: v2.2.11 | **作者**: lxfight | **许可证**: AGPLv3
...
- **会话隔离**: 支持按人格和会话隔离记忆
- **Agent 主动记忆工具**: 暴露 `recall_long_term_memory` 与 `memorize_long_term_memory`，Agent 可按需主动回忆或写入长期记忆
...
3. **Agent 记忆工具**: 为 tool loop / agent 模式提供主动回忆与主动写入能力
   - `recall_long_term_memory`: 主动回忆长期记忆，复用当前会话/人格过滤配置，返回原始记忆列表
   - `memorize_long_term_memory`: 主动写入长期记忆，始终写入当前 UMO 与当前人格，并复用自动总结的结构化存储格式
   - 适合“你还记得吗”“我之前说过什么”“请记住……”这类需要回忆或保存长期上下文的场景
...
插件会在运行时注册两个 LLM 工具，让 Agent 主动管理长期记忆：

- `recall_long_term_memory`: 主动回忆既有记忆。适合用户询问“还记得吗”“之前说过什么”，或当前指代不清、需要补查历史上下文时使用。建议用主题、实体、偏好、约定等短关键词检索。
- `memorize_long_term_memory`: 主动写入长期记忆。适合用户明确要求“记住”，或对话中出现稳定偏好、长期事实、约定、身份信息、项目背景时使用。
