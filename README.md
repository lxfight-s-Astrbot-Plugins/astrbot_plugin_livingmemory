...
- **会话隔离**: 支持按人格和会话隔离记忆
- **Agent 主动回忆**: 暴露 `search_long_term_memory` 工具，Agent 可自行选择检索时机与关键词，将结果直接带回工具上下文
...
3. **Agent 记忆工具**: 为 tool loop / agent 模式提供主动检索能力
...
除了原有的每轮自动记忆召回外，插件现在还会在运行时注册一个 LLM 工具：`search_long_term_memory`。

这个工具的特点：

- Agent 可以自己决定是否检索长期记忆，而不是只能依赖当前轮消息作为查询词
- 工具查询范围自动继承当前配置中的会话隔离与人格隔离设置
...
- 优先检索主题、实体名、偏好、约定、历史事件等高信息量词语
- 如果第一次检索结果不理想，可以换一个更具体或更抽象的关键词再次检索
