...
- Python 3.10+
- Среда разработки AstrBot (для разработки официальных Plugin Pages требуется AstrBot >= 4.24.2)
- Git
...
├── pages/                          # Ресурсы официальных страниц AstrBot Plugin Pages
│   └── dashboard/                  # Официальная страница плагина dashboard
├── webui/                          # Старый автономный WebUI для совместимости
│   └── server.py                   # FastAPI-сервер
├── static/                         # Статические ресурсы старого автономного WebUI
│   ├── index.html
│   ├── styles.css
│   ├── app.js
...
| page_api.py | Адаптер Web API для официальной страницы плагина | memory_engine, graph_store |
| tools/ | Инкапсуляция инструментов Agent | memory_engine, config_manager |

---

## Заметки по официальным Plugin Pages

- `pages/dashboard/` является входом для официальной страницы плагина AstrBot и работает внутри ограниченного iframe.
- Фронтенд должен взаимодействовать с хостовым Dashboard через `window.AstrBotPluginPage`.
- Нельзя полагаться на `window.location.origin`, потому что sandbox iframe может вернуть opaque origin.
- Нельзя напрямую использовать `localStorage` или `sessionStorage` без обработки ошибок. Нужно учитывать `SecurityError`.
- Серверные маршруты регистрируются через `context.register_web_api()`, и префикс маршрута должен включать имя плагина.
- Старый автономный WebUI сохраняется в `webui/` и `static/` для обратной совместимости.
