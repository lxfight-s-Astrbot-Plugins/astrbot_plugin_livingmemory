...
class TestSearchHandler:
    ...
    async def test_test_sparse_search_exception(self):
        """测试稀疏检索测试时的异常处理"""
        self.mock_sparse_retriever.search = AsyncMock(
            side_effect=Exception("稀疏检索错误")
        )
    ...
    def test_format_search_results_for_display_failure(self):
        """测试格式化搜索结果显示（失败）"""
        mock_response = {"success": False, "message": "搜索失败"}
    ...
    def test_format_sparse_results_for_display_failure(self):
        """测试格式化稀疏检索结果显示（失败）"""
        mock_response = {"success": False, "message": "稀疏检索失败"}
    ...
    def test_format_sparse_results_for_display_no_metadata(self):
        ...
        mock_response = {
            ...
            "data": [
                {"doc_id": 1, "score": 0.8, "content": "稀疏检索结果", "metadata": {}}
            ],
        }
