...
@pytest.mark.asyncio
async def test_handle_memory_recall_truncates_injection_with_budget(
    memory_engine,
):
    """启用注入预算时，超长 persona_summary 应被截断且头尾标记完整。"""
    from astrbot_plugin_livingmemory.core.base.config_manager import ConfigManager
    from astrbot_plugin_livingmemory.core.event_handler import EventHandler
        ...
        config_manager=ConfigManager(
            {
                "recall_engine": {
                    "top_k": 3,
                    "injection_method": "user_message_before",
                    "injection_budget_chars": 300,
                    "injection_min_chars_per_memory": 100,
                    "injection_max_chars_per_memory": 300,
                },
                "reflection_engine": {"summary_trigger_rounds": 1},
                "session_manager": {"max_messages_per_session": 100},
            }
    ...
    long_summary = "这是一段用于测试预算截断的记忆内容。" * 100  # 约 1700 字符额度
    original_metadata = {"importance": 0.9, "persona_summary": long_summary}
    recalled = Mock(
        content="ignored", final_score=0.8, metadata=original_metadata
    )
    memory_engine.search_memories = AsyncMock(return_value=[recalled])
    ...
    # 注入正文被截断：完整正文不再出现，且以省略号结尾
    assert long_summary not in req.prompt
    assert "<RAG-Faiss-Memory>" in req.prompt
    assert "…" in req.prompt
    # 元数据行与头尾标记保持完整
    assert "Importance" in req.prompt
    # 原始 metadata 对象不被污染（检索缓存安全）
    assert original_metadata["persona_summary"] == long_summary


@pytest.mark.asyncio
async def test_handle_memory_recall_budget_disabled_keeps_full_content(
    memory_engine,
):
    """injection_budget_chars=0 时关闭预算控制，注入完整正文。"""
    from astrbot_plugin_livingmemory.core.base.config_manager import ConfigManager
    from astrbot_plugin_livingmemory.core.event_handler import EventHandler

    long_summary = "这是一段用于测试预算截断的记忆内容。" * 100
    ...
    recalled = Mock(
        content="ignored",
        final_score=0.8,
        metadata={"importance": 0.9, "persona_summary": long_summary},
    )
    memory_engine.search_memories = AsyncMock(return_value=[recalled])
    ...
    # 关闭预算时正文完整注入，不被截断
    assert long_summary in req.prompt
    assert "<RAG-Faiss-Memory>" in req.prompt
