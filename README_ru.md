...
**Версия**: v2.2.10 | **Автор**: lxfight | **Лицензия**: AGPLv3
...
- **Проактивное вспоминание Agent**: Предоставляется инструмент `recall_long_term_memory`, позволяющий Agent самостоятельно выбирать момент и ключевые слова для вспоминания, возвращая результаты непосредственно в контекст инструмента
...
- **Управление через WebUI**: Визуальный интерфейс управления памятью с поддержкой двух языков (EN/ZH) и тёмной темы
...
### Требования к версии AstrBot

- Для использования **официальной панели AstrBot Plugin Pages** требуется **AstrBot >= 4.24.2**
- Совместимый вход в старый автономный WebUI не зависит от Plugin Pages и по-прежнему доступен через `/lmem webui`

### Вход в интерфейс управления

Рекомендуемый способ:

1. Откройте официальный WebUI AstrBot
2. Перейдите в `Plugins -> LivingMemory -> Pages -> dashboard`

Совместимый способ:

- Выполните `/lmem webui`
- Если старый автономный WebUI включен, откройте адрес из ответа команды
...
3. **Инструмент памяти Agent**: Предоставляет возможность проактивного восстановления памяти для режима tool loop / agent
   - Название инструмента: `recall_long_term_memory`
   - Повторное использование существующей конфигурации изоляции сессий и персон
   - Возвращает исходный список воспоминаний без дополнительной инъекции промпта
   - Подходит для сценариев типа «Ты ещё помнишь?», «Я говорил раньше...», «Помоги вспомнить»
...
## Проактивное восстановление памяти Agent

Помимо автоматического восстановления памяти, плагин регистрирует инструмент LLM во время выполнения: `recall_long_term_memory`.

Особенности этого инструмента:

- Agent может самостоятельно решать, восстанавливать ли долгосрочную память, а не полагаться только на сообщение текущего раунда в качестве запроса
- Область восстановления инструмента автоматически наследует текущие настройки изоляции сессий и персон
- Результаты поиска возвращаются как выходные данные инструмента в контекст agent и не проходят повторно через цепочку инъекции промпта памяти
- Наиболее подходит, когда пользователь просит «вспомнить», «вспомнить» или «о чём раньше говорили», или когда ссылки неоднозначны и требуется проверка исторического контекста

Рекомендуемая стратегия вызова:

- Используйте короткие ключевые слова вместо копирования полного пользовательского ввода
- Отдавайте приоритет темам, именам сущностей, предпочтениям, договорённостям или историческим событиям — словам с высокой информативностью
- Если первое восстановление неудачно, измените ключевое слово на более конкретное или более абстрактное и попробуйте снова

Возвращаемый результат — исходный список воспоминаний, включающий содержание воспоминания, оценку релевантности, важность и метаданные сессии/персоны, что позволяет agent легко определить, какие результаты действительно релевантны
...
- [API-документация](docs/API_ru.md): Подробный API-справочник
- [Документация по архитектуре](docs/ARCHITECTURE_ru.md): Описание системной архитектуры
- [Руководство разработчика](docs/DEVELOPMENT_ru.md): Руководство по разработке и внесению вклада
