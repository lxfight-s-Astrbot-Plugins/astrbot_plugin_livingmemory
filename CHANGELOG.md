...
### 修复
- **总结质量恢复**: 移除内置提示词中的 `canonical_summary` 双摘要要求，模型一次只生成一份人格化 `summary`；检索与注入内容改由代码从 `summary` + `key_facts` 组装，修复 2.4.0 以来检索语料与召回注入内容变薄、总结质量下降的问题；自定义提示词输出的 `canonical_summary` 仍被兼容保留（供图抽取等消费方使用）
