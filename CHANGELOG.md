...
### 性能
- **图谱渲染优化**: 选中节点聚焦只平移视口，不再重算整轮力布局；节点/边结构未变时复用布局缓存；空间网格改用数值键并跨迭代复用桶数组；中等及以上图（>60 节点）改为渐进式布局（分片迭代、边算边显示，不阻塞主线程）；空闲环境漂浮动画按图规模降帧渲染；边命中测试加 AABB 预过滤；背景点阵/网格缓存到离屏画布（平移只改 blit 偏移）；标签碰撞检测由 O(L²) 改为空间网格 O(L)
...
### 测试
- 新增图谱布局前端测试（同步/渐进式/Worker/回退/布局缓存/聚焦不重算/快速双加载，共 7 项）；完整 Python 测试集 743 项通过，前端测试 14 项通过
