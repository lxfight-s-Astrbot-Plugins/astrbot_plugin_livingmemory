...
### 性能
- **图谱渲染优化**: 选中节点聚焦只平移视口，不再重算整轮力布局；节点/边结构未变时复用布局缓存；空间网格改用数值键并跨迭代复用桶数组；大图改为渐进式布局（分片迭代、边算边显示，不阻塞主线程）
...
### 测试
- 新增图谱布局前端测试（同步/渐进式布局、布局缓存、聚焦不重算）；完整 Python 测试集 743 项通过，前端测试 11 项通过
