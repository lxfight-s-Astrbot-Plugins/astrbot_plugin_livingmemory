...
### 修复
- **知识图谱页面不再默认加载全量图 (#248)**: 打开「知识图谱」页（Dashboard 默认落地页）时默认加载受限概览——最近记忆的局部子图，节点/边受后端上限约束（默认 48 节点/72 边），不再一次性绘制全部节点与关系导致视觉毛团；全量图仅在用户显式点击「全量图谱」按钮时加载，空检索框提交也改为走受限概览
...
### 测试
- 新增图谱页默认受限概览与「全量图谱」按钮显式加载全量的前端行为测试（`tests/frontend/graph-ui.test.js`）
