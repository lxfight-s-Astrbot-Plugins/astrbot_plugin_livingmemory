...
### 修复
- **双通道摘要与注入**: 独立生成事实型 `canonical_summary` 用于检索，注入优先使用 `persona_summary`，避免关键事实重复注入 (#202)
- **FAISS 加载兼容性**: 移除插件模块加载阶段的 FAISS 导入，并在优化扩展不可用时尝试 generic 指令集模式 (#153, #198)
- **图记忆磁盘写放大**: 图向量索引改用批量新增和按来源记忆批量删除，避免每条图 entry 都重写完整 FAISS 索引 (#197)
