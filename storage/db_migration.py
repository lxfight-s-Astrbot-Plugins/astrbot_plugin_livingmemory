...
class DBMigration:
    ...
    async def get_db_version(self) -> int:
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                ...
                # 读取版本号
                cursor = await db.execute("SELECT version FROM db_version ORDER BY id DESC LIMIT 1")
    ...
    async def set_db_version(self, version: int, description: str = "", duration: float = 0.0):
        """
        设置数据库版本
    ...
    async def needs_migration(self) -> bool:
        ...
        if needs_migration:
            logger.warning(f"⚠️ 数据库需要迁移: v{current_version} -> v{self.CURRENT_VERSION}")
        else:
    ...
    async def migrate(
        ...
        async with self.migration_lock:
            ...
            try:
                ...
                logger.info(f"🔄 开始数据库迁移: v{current_version} -> v{self.CURRENT_VERSION}")
