...
from .sparse_retriever import SparseResult
...
class ResultFusion:
    ...
    def __init__(self, strategy: str = "rrf", config: Dict[str, Any] = None):
        ...
        # 新增参数
        self.convex_lambda = self.config.get(
            "convex_lambda", 0.5
        )  # Convex Combination 参数
    ...
    def _rrf_fusion(
        self, dense_results: List[Result], sparse_results: List["SparseResult"], k: int
    ) -> List[SearchResult]:
    ...
    def _cascade_fusion(
        ...
        # 从密集结果中筛选候选
        dense_candidates = [r for r in dense_results if r.data["id"] in candidate_ids]

        # 如果密集结果不足，用稀疏结果补充
        if len(dense_candidates) < k:
            candidate_ids.update(r.data["id"] for r in dense_candidates)
            additional_sparse = [
                r for r in sparse_results if r.doc_id not in candidate_ids
    ...
    def _dense_to_search_results(
        self, dense_results: List[Result], k: int
    ) -> List[SearchResult]:
    ...
    def _interleave_fusion(
        ...
        # 交替选择结果
        while len(final_results) < k and (
            dense_idx < len(dense_results) or sparse_idx < len(sparse_results)
        ):
            # 优先从密集结果中选择
            if (
                len([r for r in final_results if r.dense_score is not None])
                < dense_count
            ):
            ...
            if isinstance(result, Result):
                fusion_results.append(
                    SearchResult(
                        ...
                        sparse_score=getattr(result, "score", None)
                        if sparse_rank != float("inf")
                        else None,
    ...
    def _score_fusion(
        ...
        for doc_id in all_doc_ids:
            ...
            if doc_id in dense_scores:
                dense_rank = sum(
                    1
                    for other_score in dense_scores.values()
            ...
            if doc_id in sparse_scores:
                sparse_rank = sum(
                    1
                    for other_score in sparse_scores.values()
    ...
    def _apply_diversity_bonus(
        self, scores: Dict[int, float], result_map: Dict[int, Any], bonus: float
    ):
        """应用多样性奖励，避免结果过于相似"""
        # 简单的多样性策略：基于内容长度差异
        contents = {}
        ...
        if content_lengths:
            ...
            for doc_id, length in contents.items():
                diversity_factor = (
                    abs(length - avg_length) / avg_length if avg_length > 0 else 0
                )
