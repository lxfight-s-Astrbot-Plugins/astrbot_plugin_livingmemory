...
class MemoryHandler(BaseHandler):
    ...
    async def edit_memory(
        self, memory_id: str, field: str, value: str, reason: str = ""
    ) -> Dict[str, Any]:
        """编辑记忆内容或元数据"""
        ...
        try:
            ...
            if field == "content":
                ...
                if value not in valid_types:
                    return self.create_response(
                        False, f"无效的事件类型，必须是: {', '.join(valid_types)}"
                    )
                ...
                if value not in valid_statuses:
                    return self.create_response(
                        False, f"无效的状态，必须是: {', '.join(valid_statuses)}"
                    )
                updates["status"] = value
            else:
                return self.create_response(
                    False,
                    f"未知的字段 '{field}'，支持的字段: content, importance, type, status",
                )
    ...
    async def get_memory_details(self, memory_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            details = {
                ...
                "last_access_time": self.format_timestamp(
    ...
    def format_memory_details_for_display(self, details: Dict[str, Any]) -> str:
        ...
        response_parts.append("\n" + "=" * 50)
        response_parts.append("\n🛠️ 编辑指引:")
        response_parts.append("使用以下命令编辑此记忆:")
        response_parts.append("\n• 编辑内容:")
        ...
        response_parts.append("\n• 编辑类型:")
        response_parts.append(
            f"  /lmem edit {data['id']} type <FACT/PREFERENCE/GOAL/OPINION/RELATIONSHIP/OTHER> [原因]"
        )
        response_parts.append("\n• 编辑状态:")
        response_parts.append(
            f"  /lmem edit {data['id']} status <active/archived/deleted> [原因]"
        )

        # 示例
        response_parts.append("\n💡 示例:")
        response_parts.append(
            f"  /lmem edit {data['id']} importance 0.9 提高重要性评分"
        )
        response_parts.append(
            f"  /lmem edit {data['id']} type PREFERENCE 重新分类为偏好"
        )
