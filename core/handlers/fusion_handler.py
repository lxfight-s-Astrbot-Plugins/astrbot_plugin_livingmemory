...
class FusionHandler(BaseHandler):
    ...
    async def manage_fusion_strategy(
        self, strategy: str = "show", param: str = ""
    ) -> Dict[str, Any]:
        """管理检索融合策略"""
        ...
        if strategy == "show":
            ...
            update_result = await self._update_recall_engine_fusion_config(
                strategy, self.config["fusion"]
            )
            ...
            return self.create_response(
                True,
                f"融合策略已从 '{old_strategy}' 更新为 '{strategy}'{f' (参数: {param})' if param else ''}",
            )

        else:
            return self.create_response(
                False, "不支持的融合策略。使用 show 查看可用选项。"
            )
    ...
    async def test_fusion_strategy(self, query: str, k: int = 5) -> Dict[str, Any]:
        ...
        try:
            # 执行搜索
            session_id = (
                await self.context.conversation_manager.get_curr_conversation_id(None)
            )
            ...
            return self.create_response(
                True, f"融合测试完成，找到 {len(results)} 条结果", test_data
            )
    ...
    async def _process_fusion_param(self, param: str, strategy: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if key not in valid_params:
                return self.create_response(
                    False,
                    f"无效的参数名: {key}。支持的参数: {', '.join(sorted(valid_params))}",
                )
            ...
            if strategy in strategy_params and key not in strategy_params[strategy]:
                return self.create_response(
                    False, f"参数 {key} 不适用于策略 {strategy}"
                )

            # 权重和检查（对于需要权重的策略）
            if key in ["dense_weight", "sparse_weight"]:
                other_key = "sparse_weight" if key == "dense_weight" else "dense_weight"
                other_value = self.config["fusion"].get(
                    other_key, 0.3 if other_key == "sparse_weight" else 0.7
                )

                # 如果设置了新的权重，检查和是否超过1.0
                if key + other_key in [
                    k
                    for k in strategy_params.get(strategy, [])
                    if k in ["dense_weight", "sparse_weight"]
                ]:
                    total_weight = param_value + other_value
                    if total_weight > 1.0:
                        return self.create_response(
                            False,
                            f"权重总和不能超过 1.0 (当前总和: {total_weight:.2f})",
                        )
    ...
    async def _update_recall_engine_fusion_config(
        self, strategy: str, fusion_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """更新RecallEngine的融合配置"""
        try:
            ...
            try:
                if hasattr(self.recall_engine, "result_fusion"):
                    ...
                    fusion_obj.interleave_ratio = fusion_config.get(
                        "interleave_ratio", 0.5
                    )
                    fusion_obj.rank_bias_factor = fusion_config.get(
    ...
    def format_fusion_config_for_display(self, response: Dict[str, Any]) -> str:
        ...
        if current_strategy in ["rrf", "hybrid_rrf"]:
            ...
            if current_strategy == "hybrid_rrf":
                response_parts.append(
                    f"多样性奖励: {fusion_config.get('diversity_bonus', 0.1)}"
                )
        ...
        if current_strategy in ["weighted", "convex", "rank_fusion", "score_fusion"]:
            ...
            response_parts.append(
                f"稀疏权重: {fusion_config.get('sparse_weight', 0.3)}"
            )
        ...
        if current_strategy == "interleave":
            response_parts.append(
                f"交替比例: {fusion_config.get('interleave_ratio', 0.5)}"
            )

        if current_strategy == "rank_fusion":
            response_parts.append(
                f"排序偏置: {fusion_config.get('rank_bias_factor', 0.1)}"
            )
    ...
    def format_fusion_test_for_display(self, response: Dict[str, Any]) -> str:
        ...
        for i, result in enumerate(results, 1):
            ...
            response_parts.append(
                f"   重要性: {result['importance']:.3f} | 类型: {result['event_type']}"
            )
            response_parts.append(
                f"   内容: {result['text'][:100]}{'...' if len(result['text']) > 100 else ''}"
            )

        response_parts.append("\n" + "=" * 50)
        response_parts.append("💡 当前融合配置:")
        ...
        if strategy in ["weighted", "convex"]:
            response_parts.append(
                f"   密集权重: {fusion_config.get('dense_weight', 0.7)}"
            )
            response_parts.append(
                f"   稀疏权重: {fusion_config.get('sparse_weight', 0.3)}"
            )
