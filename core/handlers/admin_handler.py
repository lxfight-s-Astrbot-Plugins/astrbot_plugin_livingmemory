...
class AdminHandler(BaseHandler):
    ...
    async def get_memory_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return self.create_response(
                True, "获取记忆库状态成功", {"total_count": count}
            )
    ...
    async def set_search_mode(self, mode: str) -> Dict[str, Any]:
        ...
        if mode not in valid_modes:
            return self.create_response(
                False, f"无效的模式，请使用: {', '.join(valid_modes)}"
            )
    ...
    async def get_config_summary(self, action: str = "show") -> Dict[str, Any]:
        ...
        if action == "show":
            try:
                ...
                config_summary = {
                    "session_manager": {
                        "max_sessions": self.config.get("session_manager", {}).get(
                            "max_sessions", 1000
                        ),
                        "session_ttl": self.config.get("session_manager", {}).get(
                            "session_ttl", 3600
                        ),
                        "current_sessions": self.session_manager.get_session_count()
                        if self.session_manager
                        else 0,
                    },
                    "recall_engine": {
                        "retrieval_mode": self.config.get("recall_engine", {}).get(
                            "retrieval_mode", "hybrid"
                        ),
                        "top_k": self.config.get("recall_engine", {}).get("top_k", 5),
                        "recall_strategy": self.config.get("recall_engine", {}).get(
                            "recall_strategy", "weighted"
                        ),
                    },
                    "reflection_engine": {
                        "summary_trigger_rounds": self.config.get(
                            "reflection_engine", {}
                        ).get("summary_trigger_rounds", 10),
                        "importance_threshold": self.config.get(
                            "reflection_engine", {}
                        ).get("importance_threshold", 0.5),
                    },
                    "forgetting_agent": {
                        "enabled": self.config.get("forgetting_agent", {}).get(
                            "enabled", True
                        ),
                        "check_interval_hours": self.config.get(
                            "forgetting_agent", {}
                        ).get("check_interval_hours", 24),
                        "retention_days": self.config.get("forgetting_agent", {}).get(
                            "retention_days", 90
                        ),
                    },
                }
            ...
            return self.create_response(
                False, "无效的动作，请使用 'show' 或 'validate'"
            )
    ...
    def format_config_summary_for_display(self, response: Dict[str, Any]) -> str:
        ...
        config_summary.append("🧠 回忆引擎:")
        config_summary.append(
            f"  - 检索模式: {re_config.get('retrieval_mode', 'hybrid')}"
        )
        config_summary.append(f"  - 返回数量: {re_config.get('top_k', 5)}")
        config_summary.append(
            f"  - 召回策略: {re_config.get('recall_strategy', 'weighted')}"
        )
        config_summary.append("")

        # 反思引擎配置
        rf_config = data.get("reflection_engine", {})
        config_summary.append("💭 反思引擎:")
        config_summary.append(
            f"  - 触发轮次: {rf_config.get('summary_trigger_rounds', 10)}"
        )
        config_summary.append(
            f"  - 重要性阈值: {rf_config.get('importance_threshold', 0.5)}"
        )
        config_summary.append("")

        # 遗忘代理配置
        fa_config = data.get("forgetting_agent", {})
        config_summary.append("🗑️ 遗忘代理:")
        config_summary.append(
            f"  - 启用状态: {'是' if fa_config.get('enabled', True) else '否'}"
        )
        config_summary.append(
            f"  - 检查间隔: {fa_config.get('check_interval_hours', 24)}小时"
        )
